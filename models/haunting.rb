class Haunting
  ...
  def initialize (params)
    ...
    @ghost_description = params['ghost_name']
  ...
  def save()
    sql = "INSERT INTO hauntings(haunting_name, haunting_description, ghost_description, ghost_type_id, location_type_id, customer_id) VALUES ('#{@haunting_name}', '#{@haunting_description}', '#{@ghost_description}', #{@ghost_type_id}, #{@location_type_id},  #{@customer_id} ) RETURNING *"
    results = SqlRunner.run(sql)
    @id = results.first()['id'].to_i
  end

  def update()
    sql = "UPDATE hauntings SET (haunting_name, haunting_description, ghost_description, ghost_type_id, location_type_id, customer_id) = ('#{@haunting_name}', '#{@haunting_description}', '#{@ghost_description}', #{@ghost_type_id}, #{@location_type_id}, #{@customer_id})"
    SqlRunner.run(sql)
  end
